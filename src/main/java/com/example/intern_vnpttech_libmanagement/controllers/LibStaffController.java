...
public class LibStaffController {
    ...
    {
        return libStaffService.findById(staffId).isPresent()
                ?ResponseEntity.status(200).body(libStaffService.findById(staffId))
    ...
    {
        Optional<LibStaff> staffOptional = libStaffService.findByUsernameOrEmailOrPhone(staffRequest.getStaffUsername(),staffRequest.getStaffEmail());
...
//        else staffDTO.setStaffId(staffId);
        return libStaffService.update(staffDTO).isPresent()
...
//        if(!libStaffService.findById(staffId).isPresent())
//            return ResponseEntity.status(204).body(new MessageResponse("Staff not found","fail"));
        String encodedCurrentPass = libStaffService.findById(staffId).get().getStaffPassword();
        ...
        return libStaffService.update(LibStaffDTO.builder()
                                                    .staffId(staffId)
    ...
    {
        return libStaffService.delete(staffId)
